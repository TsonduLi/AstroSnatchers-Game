...
bool InitTestHarnessService1(uint8_t Priority)
{
  ...
  DB_printf( "Press 'M' to test event ES_PRESS_M \n\r");
  DB_printf( "Press 'E' to test event ES_PRESS_E \n\r");
...
ES_Event_t RunTestHarnessService1(ES_Event_t ThisEvent)
{
  ...
  switch (ThisEvent.EventType)
  {
    ...
    case ES_NEW_KEY:   // announce
    {
      ...
      {
        ES_Event_t NewEvent;
        NewEvent.EventType = ES_PRESS_M;
        NewEvent.EventParam = ThisEvent.EventParam;
        PostTestHarnessService1(NewEvent);
      }
      if ('E' == ThisEvent.EventParam)
      {
        ES_Event_t NewEvent;
        NewEvent.EventType = ES_PRESS_E;
        NewEvent.EventParam = ThisEvent.EventParam;
        PostTestHarnessService1(NewEvent);
      }
#ifdef TEST_INT_POST
    ...
    case ES_PRESS_M: {
        DB_printf("\rM for MECHANIAL received in Service %d\r\n", MyPriority);
    }
    break;
    
    case ES_PRESS_E: {
        DB_printf("\rE for ENGINEERING received in Service %d\r\n", MyPriority);
    }
    break;
